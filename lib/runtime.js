...
yr.register = function(id, module) {
    ...
    // импортированные модули из тех, что мы импортируем
    var secondLevelImports = [];

    /*
     При уплощении списка очень важно сохранить порядок импортов.
     Допустим есть модуль A, его импортируют B и C. И есть модуль D, который импортирует B и С

     Если просто все сконкатить, то конечный список импортов для D будет "B A C A" и будут проблемы в шаблонизации
     Например, в A есть какие-то общие шаблоны, переопределенный в C, то вызываться будут они, а не C

     Правильный список для D "B C A"
     */
    for (var i = 0, l = ids.length; i < l; i++) {
        ...
        if (!module_) {
            ...
            imports.push(module_);
            // отрезаем первый модуль, это и есть сам импортируемый модуль
            secondLevelImports = secondLevelImports.concat(module_.imports.slice(1));
        }
    }

    // убираем дубликаты
    var uniqSecondLevelImports = [];
    for (var x = 0, y = secondLevelImports.length; x < y; x++) {
        if (uniqSecondLevelImports.indexOf(secondLevelImports[x]) === -1) {
            uniqSecondLevelImports.push(secondLevelImports[x]);
        }
    }

    imports = imports.concat(uniqSecondLevelImports);
...
yr.run = function(id, data, mode) {
    ...
    var r = module.a(module, 0, [ doc.root ], mode, { a: {} } );
