    ...
    for (var i0 = 0, l0 = nodeset.length; i0 < l0; i0++) {
        ...
        while (!found && i2 < l2) {
            ...
            if (names) {
                ...
                var tpmlName = c0.name;
                // проверяем что я - рут (/)
                if (!c0.parent) {
                    // сбрасываем имя, потому document из chroot имеют имя
                    tpmlName = '';
                }
                var templates = names[tpmlName] || names['*'];
                if (templates) {
                    var i3 = 0;
...
yr.document = function(nodeset) {
    ...
    if (!nodeset.length) {
        ...
        // добавляем name к document, чтобы удобнее матчится
        doc = new Doc( nodeset[0].data, nodeset[0].name );
    }
    return [ doc.root ];
};
