...
function isSameImportedFunctions(existsFunction, importingFunction) {
    // TODO: надо как-то понимать, что они импортированы из одного места
    return (
        // проверяем, что обе импортированы
        (existsFunction.f.isImported && importingFunction.f.isImported) &&

        // проверяем, что ID совпадают
        (existsFunction.p.Id === importingFunction.p.Id)
    )
}
...
yate.asts.function_.w_declarations = function() {
    ...
    if (functions[name]) {
        ...
        // если обе функции импортированы, то возможно они импортированы из одного места и ругаться не надо
        if ( isSameImportedFunctions(functions[name], this) ) {
            return;
    ...
    if (this.p.Id === undefined) {
        ...
        К имени функции надо приклеивать имя модуля, иначе потом нельзя будет отличить
        Есть у нас модуль B, в нем скомпилились функция f0
        Есть у нас модуль С, в нем тоже скомпилились функция f0, хотя эта другая функция

        И при множественном импорте вызываться будет не то
        ...
        Исполняем D (который import B, import C). Когда C вызовет свою функцию f0, она будет искаться по всем импортам.
        А там первым будет B, у которого тоже будет f0. И вызовется она, а не из С.
         */

        //TODO: тут все склеивается в fmod_d_mod_c_mod_a_11111
        this.p.Id = parent.p.Name + '_'+ (this.state.fid);
